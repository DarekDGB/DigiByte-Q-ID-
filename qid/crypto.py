...
internals later without changing the external API.
"""
...
# Dev algorithm (tests / examples only)
DEV_ALGO = "dev-hmac-sha256"

# PQC algorithm IDs (wire these to real implementations later)
...
# Legacy alias (kept only for backward compatibility with early docs)
# Do not use for new signatures.
_LEGACY_HYBRID_ALGO = "hybrid-dev-ml-dsa"

_ALLOWED_ALGOS = {DEV_ALGO, ML_DSA_ALGO, FALCON_ALGO, HYBRID_ALGO, _LEGACY_HYBRID_ALGO}

# ---------------------------------------------------------------------------
# Signature envelope (v1) — canonical, fail-closed
# ---------------------------------------------------------------------------
...
class QIDKeyPair:
    ...
    NOTE: In real public-key PQC, verification uses only the public key.
    In this dev/HMAC reference implementation we re-use the secret key.
    """
    algorithm: str
    secret_key: str
    public_key: str
...
def _envelope_encode(obj: Dict[str, Any]) -> str:
    """
    Encode an envelope dict as base64(canonical_json(envelope)).
    """
...
def _envelope_decode(sig: str) -> Dict[str, Any] | None:
    """
    Decode a base64(JSON) envelope. Returns None on any failure.
    """
    try:
        raw = _b64decode(sig)
...
def _normalize_alg(alg: str) -> str:
    # Map legacy hybrid ID to the new strict hybrid ID.
...
# ---------------------------------------------------------------------------
# Backend stubs (dev/HMAC only) — replace with real PQC later
# ---------------------------------------------------------------------------
...
def _stub_sign_pqc(msg: bytes, secret: bytes, alg: str) -> bytes:
    # PQC stub: HMAC-SHA512 with algorithm prefix to prevent interchange.
...
def _stub_sign_hybrid(msg: bytes, secret: bytes) -> Dict[str, bytes]:
    # Hybrid stub: split secret into two halves, sign independently.
    if len(secret) < 64:
        raise ValueError("Hybrid secret key must be at least 64 bytes")

    s1, s2 = secret[:32], secret[32:64]
    sig_ml_dsa = hmac.new(s1, msg, hashlib.sha256).digest()
    sig_falcon = hmac.new(s2, msg, hashlib.sha512).digest()
    return {ML_DSA_ALGO: sig_ml_dsa, FALCON_ALGO: sig_falcon}
...
def _stub_verify_hybrid(msg: bytes, secret: bytes, sigs: Mapping[str, bytes]) -> bool:
    ...
    s1, s2 = secret[:32], secret[32:64]
    expected_ml_dsa = hmac.new(s1, msg, hashlib.sha256).digest()
    expected_falcon = hmac.new(s2, msg, hashlib.sha512).digest()

    return hmac.compare_digest(expected_ml_dsa, sigs[ML_DSA_ALGO]) and hmac.compare_digest(
        expected_falcon, sigs[FALCON_ALGO]
    )
...
def sign_payload(payload: Dict[str, Any], keypair: QIDKeyPair) -> str:
    ...
    Returns a base64(JSON) signature envelope (v1).

    Envelope (v1):
      - Non-hybrid:
          {"v":1,"alg":"...","sig":"<b64>"}
      - Hybrid:
          {"v":1,"alg":"pqc-hybrid-ml-dsa-falcon","sigs":{"pqc-ml-dsa":"<b64>","pqc-falcon":"<b64>"}}

    This function is deterministic for the same payload dict content.
    """
    ...
    if alg == DEV_ALGO:
        ...
        env = {"v": _SIG_ENVELOPE_VERSION, "alg": DEV_ALGO, "sig": _b64encode(sig)}
...
def verify_payload(payload: Dict[str, Any], signature: str, keypair: QIDKeyPair) -> bool:
    """
    Verify a signature envelope (v1) over a payload using the given key-pair.

    Fail-closed behavior:
    - Any parse failure -> False
    - Any schema mismatch -> False
    - Any algorithm mismatch -> False
    - Hybrid requires BOTH signatures with exact keys -> False if not
    """
    env = _envelope_decode(signature)
    if env is None:
        return False

    # Strict envelope version
    ...
    if env_alg == HYBRID_ALGO:
        ...
        # Strict: only these two keys, both must be base64 strings
        if set(sigs.keys()) != {ML_DSA_ALGO, FALCON_ALGO}:
            return False
